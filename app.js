...
// FIKSIVERSE CLEAN AU READER (PAKAI SERVERLESS BACKEND API /api/unroll)
window.openCleanThreadReader = async function(tweetUrl, title) {
  ...
  bodyContainer.innerHTML = `
    <div style="text-align:center; padding:40px 15px; color:#c084fc;">
      ...
      <p style="font-size:13px; font-weight:700; margin-top:10px;">Server FiksiVerse Menyusun Naskah...</p>
      <p style="font-size:10px; color:#94a3b8; margin-top:4px;">Memuat cerita full dari awal sampai tamat ✨</p>
    </div>
  `
  ...
  try {
    // Panggil Endpoint Backend Microservice Vercel
    const res = await fetch(`/api/unroll?id=${tweetId}`)
    const data = await res.json()

    if (!res.ok || !data.images || data.images.length === 0) {
      throw new Error(data.error || 'Gagal memuat naskah')
    }

    // Render Full Naskah AU di Modal FiksiVerse
    let htmlContent = ''
    ...
    bodyContainer.innerHTML = `
      <div style="text-align:center; padding:30px 15px; color:#f87171; font-size:12px;">
        <i class="bi bi-exclamation-triangle" style="font-size:20px; display:block; margin-bottom:6px;"></i>
        Gagal memuat naskah AU.<br>
        <span style="color:#cbd5e1; font-size:10px;">Pastikan project sudah ter-deploy ke Vercel agar API backend aktif.</span>
      </div>
    `
  }
}
      ...
      detailContent.innerHTML = `
        ...
        <div class="space-y-2" style="padding-top:8px;">
          ...
          ${(() => {
            ...
            if (isAllowedReader) {
              return `
                <button onclick="openCleanThreadReader('${sanitizeText(book.read_link_2)}', '${sanitizeText(book.title)}')" class="btn-full" style="background:linear-gradient(135deg, #059669, #10b981); color:white; border:none; font-weight:700; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px;">
